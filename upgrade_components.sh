...
echo -e "${YELLOW}Upgrading Veeam components.${NC}"
echo ""
echo -e "${WHITE}You MUST use the "'"bs4it_upgrade_repo.ps1"'" PowerShell script to upgrade this repository.${NC}."
...
cp $(dirname "$0")/util/bs4it_upgrade_repo.ps1 /tmp/bs4it_upgrade_repo.ps1
...
<body>
   ...
   <h3><font color='#B22222'>PowerShell Script Download</font></h3>
   <hr>
   <font color='#000000'>
   Click the button bellow to download the PowerShell script to the VB&R console.<br>
<br>
<a href="bs4it_upgrade_repo.ps1" download="bs4it_upgrade_repo.ps1">
...
(trap 2; bash $(dirname "$0")/upgrade_components_count.sh $webserver_pid $fqdn)
# protect this script against ctrl+c
trap '' 2
clear
build_banner "UPGRADE VEEAM COMPONENTS" "bs4it@2022"
...
(rm -f /tmp/status /tmp/index.html /tmp/cgi-bin/status.py; rmdir /tmp/cgi-bin)
sleep 0.3
